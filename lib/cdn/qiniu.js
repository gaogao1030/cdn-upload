...
const upload = (files, opts) => {
  let file = undefined
  if(files.length == 0){
    print("No files need upload")
    return new Promise((resolve, reject) => {
      resolve({ failedFiles, uploadedFiles })
    })
  }
  return new Promise((resolve, reject) => {
    _Promise.map(files, (file) => {
      return new Promise(async (_resolve, _reject) => {
        try {
          ...
          print(`Upload success:\t${path.join(remoteDirectory, file)}`)
...
const remove = (files) => {
  return new Promise(async (resolve, reject) => {
    print(`********** Ready to remove files **********`)
    if(files.length == 0) { 
      print("No files need remove")
      resolve() 
    } else {
      print("Please wait while removing files...")
      ...
      try {
        ...
        if(parseInt(respInfo.statusCode / 100) == 2) {
          ...
          respBody.forEach((item) => {
            if(item.code == 200) {
              print(`Remove success:\t${files[i]}`)
            } else {
              print(`${item.code}\t${item.data.error}`)
            }
            i = i + 1
          })
        } else {
          print(respInfo.statusCode)
          print(respBody)
        }
        resolve()
      } catch (err) {
    ...
    if(keys.length == 0) { resolve() } else {
      ...
      try {
        ...
        if(parseInt(respInfo.statusCode / 100) == 2) {
          ...
          respBody.forEach((item) => {
            if(item.code == 200) {
              print(`${keys[i]} MimeTypeChanged:\t${mimeTypes[keys[i]]}`)
            } else {
              print(`${item.code}\t${item.data.error}`)
            }
            i = i + 1
